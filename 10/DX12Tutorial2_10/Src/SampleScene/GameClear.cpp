...
*/
::Scene::ScenePtr GameClearScene::Create()
{
  return ::Scene::ScenePtr(new GameClearScene);
}

/**
* コンストラクタ.
*/
GameClearScene::GameClearScene() : Scene(L"Title")
{
}
...
* シーンを読み込む.
*/
bool GameClearScene::Load(::Scene::Context&)
{
  ...
  spriteList.back().color = XMFLOAT4(0.5f, 0.5f, 1.0f, 1.0f);
  const char start[] = "GAME CLEAR";
  XMFLOAT3 pos(center.x - 16.0f * 5.0f, center.y, 0.5f);
  XMVECTORF32 color = { 0.1f, 0.9f, 0.9f, 1 };
  const XMVECTORF32 colorFactor = { 0.9f, 0.8f, 1.0f, 1 };
...
*/
int GameClearScene::Update(::Scene::Context&, double delta)
{
...
* シーンを描画する.
*/
void GameClearScene::Draw(Graphics::Graphics& graphics) const
{
